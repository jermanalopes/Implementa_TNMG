function [SNR_overall, SNR_overall_type] = average_beats_tnmg(ecg,R_pos,fs,vcg) 
...
SNR_overall_type=0;
tam_R=length(R_pos);
mediana_R = double(round(median(diff(R_pos))*1.1));
offset=round(mediana_R/3);
beats=NaN.*zeros(tam_R,size(ecg,1),mediana_R);
beats_vcg=NaN.*zeros(tam_R,2,mediana_R);
R_PosAtual = double([R_pos length(ecg)]); %Acrescenta o length ao R_Pos
group=ones(1,tam_R);
int0 =0:(mediana_R-1);
...
%             errteste(j)= nanmean(nanmean((squeeze(beats(i,1:3,j:end))-beats_av(1:3,1:(end-j+1))).^2,2));
            ...
            if err1<err, err=err1; pos=double(j-1); end %Guarda a pos da janela mdc para o menor erro
            if err2<err, err=err2; pos=double(-(j-1)); end 
        end
...
if (R_pos(end)-R_pos(1))/sum(~isnan(ecg(1,:)))<length_thr
    SNR_median=zeros(size(beats_av,1),1);
    SNR_overall=0;
    SNR_overall_type=3;
    %SNR_overall_type=1; 
else
    err=zeros(size(ecg,1),tam_R);%Zera os erros e recalcula
    for i=1:tam_R %Calcula a media da dif entre beats e beats_av
        err(:,i)=nanmean((squeeze(beats(i,:,:))-beats_av).^2,2);
    end
    %Calculo SNR
    SNR_median=nanvar(beats_av,[],2)./nanmedian(err(:,group==1),2);
    SNR_median(isnan(SNR_median))=0; %NaN ? flat line
    der_nulas = (SNR_median<=0.20);
    if length(SNR_median)==12 %Verifica se o exame ta completo
...
%% Calculo SNR.overall type 0, 1 e 3
    SNR_overall=median([SNR_median; SNR_median(important_leads)]);% Coloca como relevante das der (1, 2, 6)
%     if(SNR_overall < SNR_threshold)
%         SNR_overall = 1;
%     else
%          SNR_overall = 0;
%     end
%     SNR_overall_type=double(SNR_overall<SNR_threshold); %0 means good 1 means bad quality. SNR_threshold definido no inicio e = 7
    very_bad_lead=SNR_median<(SNR_threshold/3);
